...
$rs_banners = p4c_query("SELECT * FROM `ads_media` WHERE `new_filename`!='' AND `rejected`='0';", __FILE__, __LINE__);
...
if (isset($_GET['delete'])) {
    ...
    header("Location: ".ACP_URL.'/Neue-Banners');
    exit;    
}

if (isset($_GET['rejected'])) {
    $rs_banner = p4c_query("SELECT * FROM `ads_media` WHERE `file_id`='". p4c_escape_string($_GET['rejected'])."' LIMIT 1;", __FILE__, __LINE__);
    if (p4c_num_rows($rs_banner) > 0) {
        p4c_query("UPDATE `ads_media` SET
            `rejected` = '3'
        WHERE  `file_id`='". p4c_escape_string($_GET['rejected'])."' LIMIT 1;",__FILE__,__LINE__);        
    }
    header("Location: ".ACP_URL.'/Neue-Banners');
    exit;  
}    
...
if (isset($_GET['unlock'])) {
    $rs_banner = p4c_query("SELECT * FROM `ads_media` WHERE `file_id`='". p4c_escape_string($_GET['unlock'])."' LIMIT 1;", __FILE__, __LINE__);
    if (p4c_num_rows($rs_banner) > 0) {
        $banner_obj = p4c_fetch_object($rs_banner);
        ...
        if (is_file($path_new_filename)) {
            p4c_query("UPDATE `ads_media` SET
                `filename`='". p4c_escape_string($banner_obj->new_filename)."',
                `new_filename` = '',
                `upload_datetime`='".date("y-m-d H:i:s")."',
                `rejected` = '0'
            WHERE  `file_id`='". p4c_escape_string($_GET['unlock'])."' LIMIT 1;",__FILE__,__LINE__);
            
            if ($banner_obj->filename != $banner_obj->new_filename) {
                $path_old_filename = ADS_PATH.'/'.ADS_DEFAULT_DIR.'/'.$banner_obj->site_id.'/'.$banner_obj->filename;
                unlink($path_old_filename);
            }
        }
    }
        ...
        // <![CDATA[
        	jQuery(document).ready(function() {
            	...
            	oTable = jQuery("#table_banners").dataTable({
                    ...
                    "bProcessing": true,
                    //"bServerSide": true,
                    "sAjaxSource": "'.ACP_URL.'/includes/ajax/banners_new.php",
