...
if (isset($_POST['cid'])) {
    ...
    $rs_clicks = p4c_query("SELECT * FROM `ads_clicks_".$month."` WHERE
...
if (p4c_num_rows($rs_clicks) > 0) {
    ...
    <div style="margin:20px 0;">
        <table id="table_refs" style="width:100%;">
            ...
            while($clicks_obj = p4c_fetch_object($rs_clicks)) {
                $site .= '
                <tr>
                    <td>'.$clicks_obj->timestamp.'</td>
                    <td>'.$clicks_obj->referer.' <a href="'.$clicks_obj->referer.'" target="_blank"><i class="material-symbols-outlined" style="font-size: 18px; vertical-align: sub;">open_in_new</i></td>
                </tr>
                ';
            }
            echo '
